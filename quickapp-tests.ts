...
record.onframerecorded = function(data) {
  data.frameBuffer === new ArrayBuffer();
...
NFCAdp.onDiscovered({
  callback(data) {
    data.id === new ArrayBuffer();
    data.techs[0] === 'NFC_A';
    data.messages[0].id === new ArrayBuffer();
    data.messages[0].type === new ArrayBuffer();
    data.messages[0].payload === new ArrayBuffer();
  },
});
